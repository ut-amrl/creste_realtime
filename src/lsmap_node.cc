...
std::tuple<torch::Tensor, torch::Tensor> LSMapNode::projection(
  ...
  for (size_t i = 0; i < num_points; ++i) {
    ...
    float z = pixels(2, i) * 1000.0f; // Convert to mm
  ...
  // RCLCPP_INFO(this->get_logger(), "Min depth: %lf, Max depth: %lf", min_depth,
  //             max_depth);
  ...
  Eigen::Matrix<float, 4, 4> pixel2point_T = pixel2point.transpose(); // Eigen is column-major, so we need to transpose

  torch::Tensor pixel2point_tensor =
      torch::from_blob(pixel2point_T.data(), {4, 4}, torch::kFloat32).clone();
...
void LSMapNode::tensorToGridMap(
  ...
  for (const auto& item : output) {
    ...
    if (!is_map_set) {
      ...
      map.setGeometry(grid_map::Length(map_width, map_height), grid_res, grid_map::Position(0,0));
...
std::tuple<at::Tensor, at::Tensor> LSMapNode::computePCA(
  ...
  for (int i = 0; i < cpu_tensor.dim(); i++) {
    RCLCPP_INFO(this->get_logger(), "Tensor dim %d: %ld", i, cpu_tensor.size(i));
  }
...
void LSMapNode::run() {
  ...
  //       pca_result.min());
  // auto rgb_tensor = pca_result.reshape({semantic.size(0), 3, semantic.size(2), semantic.size(3)}); // [B, 3, H, W]
  // // Convert tensor to rgb image
  // cv::Mat rgb_mat(semantic.size(2), semantic.size(3), CV_32FC3, rgb_tensor.data_ptr<float>());
